...
--- Retrive all employees whose salary is greater tan the highest salary of the HR department
-- SELECT * FROM employees WHERE salary > 63000;
SELECT max(salary) FROM employees WHERE department_name = 'HR'; --scaler
SELECT * FROM employees 
...
SELECT * FROM (SELECT department_name, sum(salary) from employees GROUP BY department_name) as sum_dept_name;
